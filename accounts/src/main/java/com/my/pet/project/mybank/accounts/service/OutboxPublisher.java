...
public class OutboxPublisher {
    ...
    @SchedulerLock(name = "publishOutboxEvents_accounts", lockAtMostFor = "4s")
    public void publishEvents() {
        List<OutboxEvent> events = outboxEventRepository.findBySentFalse();
        for (OutboxEvent event : events) {
            try {
                kafkaTemplate.send("notifications.accounts", String.valueOf(event.getId()), event.getPayload()).get();
